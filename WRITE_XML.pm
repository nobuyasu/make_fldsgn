...
sub score {
  ...
  if ( $mode eq "BUILD" ) {
    ...
    if( $ssmap->{term_rebuild} ) {
      ...
      print $fh "  < Reweight scoretype=omega weight=0.1 />\n";
      print $fh "</SFXN${num}>\n\n";
    }
...
sub filter {
    ...
    my $this = shift;
    my ( $fh, $mode, $num, $init_close, $bbskel, $ssmap ) = @_;
    ...
    if ( $mode eq "BUILD" ) {
      ...
      print $fh "  <SheetTopology name=st${num} blueprint=\"$bbskel\" />\n";
      ...
      if( $ssmap->{term_rebuild} ) {
        ...
        print $fh "  <HelixKink     name=hk${num} blueprint=\"$bbskel1\" />\n";
        print $fh "  <SecondaryStructure name=ss${num} blueprint=\"$bbskel1\" use_abego=1/>\n";
        ...
        print $fh "  <HelixKink     name=hk${num} blueprint=\"$bbskel\" />\n";
        print $fh "  <SecondaryStructure name=ss${num} blueprint=\"$bbskel\" use_abego=1/>\n";
        print $fh "  <CompoundStatement name=ft${num} >\n";
        print $fh "    <AND filter_name=hk${num}/>\n";
      ...
      print $fh "##${num}:SHEET-REFINE\n";
      print $fh "  <SecondaryStructure name=ft${num} blueprint=\"$bbskel\" />\n\n";
      ...
      print $fh "  <SecondaryStructure name=ft${num} blueprint=\"$bbskel\" use_abego=1/>\n";
...
sub mover {
  ...
  my $this = shift;
  my ( $fh, $mode, $num, $init_close, $bbskel, $ssmap, $resfile ) = @_;
  ...
  if ( $mode eq "BUILD" || $mode eq "REBUILD" ) {
    ...
    print $fh "##${num}:SHEET-REFINE\n";
    print $fh "  <FlxbbDesign ncycles=1 name=design${num} constraints_sheet=10.0 blueprint=\"$bbskel\" sfxn_design=SFXN${num} sfxn_relax=SFXN${num} task_operations=limitchi2,layer_all filter_trial=1/>\n";
    ...
    print $fh "  <LoopOver name=sheet_refine mover_name=sheet_refine_move${num} filter_name=ft${num} iterations=3 drift=1 ms_whenfail=FAIL_DO_NOT_RETRY/>\n\n";

    push ( @{$this->{movers}}, "sheet_refine_move${num}" );
